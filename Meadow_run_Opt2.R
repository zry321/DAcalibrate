...
setwd("/lunarc/nobackup/projects/snic2020-6-29/Ruiying/CENTURY/CENTURY_FUTURE/CENTURY_file/century_only_terra_1006/caodian_0713_SOCD1")
...
pars_new <- read.csv("caodian_SOCFluxbased_SOC_pars_1006.csv")
...
for (times in 1:5){
updated_num <- nrow(pars_new)-sum(is.na(pars_new$V1))-times+1

k<-1
for (para in 1:nrow(DA_parameters)){
  if(paraTemp_origin$DA[para]==1){
    paraTemp_origin[para,"Default"] <- pars_new[updated_num,k]
    k<-k+1
  }
}
...
save(caodian_result,file=paste0("caodian_SOCFluxbased_result_",times,".RData"))
}
